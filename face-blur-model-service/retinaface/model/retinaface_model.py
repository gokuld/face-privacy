...
def load_weights(model):
    ...
    url = "https://github.com/serengil/deepface_models/releases/download/v1.0/retinaface.h5"  # noqa: E501
...
def build_model():
    ...
    bn_data = BatchNormalization(
        epsilon=1.9999999494757503e-05, name="bn_data", trainable=False
    )(data)
    ...
    bn0 = BatchNormalization(
        epsilon=1.9999999494757503e-05, name="bn0", trainable=False
    )(conv0)
    ...
    bn1 = BatchNormalization(
        epsilon=1.9999999494757503e-05, name="bn1", trainable=False
    )(plus15)
    ...
    ssh_m3_det_context_conv1_pad = ZeroPadding2D(padding=tuple([1, 1]))(
        ssh_c3_lateral_relu
    )
    ...
    ssh_m2_det_context_conv1_pad = ZeroPadding2D(padding=tuple([1, 1]))(
        ssh_c2_aggr_relu
    )
    ...
    ssh_m2_red_up = UpSampling2D(
        size=(2, 2), interpolation="nearest", name="ssh_m2_red_up"
    )(ssh_c2_aggr_relu)
    ...
    inter_1 = concatenate(
        [
            face_rpn_cls_score_stride32[:, :, :, 0],
            face_rpn_cls_score_stride32[:, :, :, 1],
        ],
        axis=1,
    )
    inter_2 = concatenate(
        [
            face_rpn_cls_score_stride32[:, :, :, 2],
            face_rpn_cls_score_stride32[:, :, :, 3],
        ],
        axis=1,
    )
    ...
    ssh_m1_det_context_conv1_pad = ZeroPadding2D(padding=tuple([1, 1]))(
        ssh_c1_aggr_relu
    )
    ...
    inter_1 = concatenate(
        [
            face_rpn_cls_score_stride16[:, :, :, 0],
            face_rpn_cls_score_stride16[:, :, :, 1],
        ],
        axis=1,
    )
    inter_2 = concatenate(
        [
            face_rpn_cls_score_stride16[:, :, :, 2],
            face_rpn_cls_score_stride16[:, :, :, 3],
        ],
        axis=1,
    )
    final = tf.stack([inter_1, inter_2])
    ...
    inter_1 = concatenate(
        [
            face_rpn_cls_score_stride8[:, :, :, 0],
            face_rpn_cls_score_stride8[:, :, :, 1],
        ],
        axis=1,
    )
    inter_2 = concatenate(
        [
            face_rpn_cls_score_stride8[:, :, :, 2],
            face_rpn_cls_score_stride8[:, :, :, 3],
        ],
        axis=1,
    )
    final = tf.stack([inter_1, inter_2])
